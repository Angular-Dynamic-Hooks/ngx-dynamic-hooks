...
Simply put, hooks are any piece of text in the dynamic content to be replaced by an Angular component. 

Hooks can be **standalone** (`<hook>`) or **enclosing** (`<hook>...</hook>`). To find them, each hook has a corresponding **HookParser** that looks for it and tells the library how to instantiate the component.
...
Please not that this library does not aim to be a full Angular template parser. It merely looks for all registered hooks and replaces them with their corresponding Angular components, nothing more. 
