...
* **Pattern flexibility:** You are not limited to load components by their selector HTML tags. A hook can have any form and doesn't have to be an HTML element at all. You can automatically replace anything you want with a component (see ["emoji" example](#72-example-emoji-parser-standalone) or ["link" example](#73-example-internal-link-parser-enclosing)).
* **Scope:** When using Angular elements, web components will automatically load from anywhere in the DOM as they are globally registered with the browser. With this library, you can to specify on each `OutletComponent` individually which components to look for, allowing for multiple custom-tailored configurations according to your use-case.
* **Communication:** In Angular elements, there is no direct line of communication between the parent component rendering the dynamic content and dynamic components loaded as children (such as the [context object](#51-context--dependency-injection) from this library). You will have to fallback on services to transfer data.
