...
describe('forChild', () => {
  ...
  function testModuleRoutingScaffolding(setup: TestSetup): void {
    ...
    router.navigate(['planets/m', {outlets: {nestedOutlet: 'countries'}} ]);
    tick();
    setup.fixture.detectChanges();
    expect(setup.fixture.nativeElement.innerHTML).not.toContain('Hyperlanes component exists');
    expect(setup.fixture.nativeElement.innerHTML).not.toContain('Stars component exists');
    ...
    router.navigate(['planets/m', {outlets: {nestedOutlet: 'cities'}} ]);
    tick();
    setup.fixture.detectChanges();
    expect(setup.fixture.nativeElement.innerHTML).not.toContain('Hyperlanes component exists');
    expect(setup.fixture.nativeElement.innerHTML).not.toContain('Stars component exists');
    ...
    router.navigate(['planets/m', {outlets: {nestedOutlet: 'species'}} ]);
    tick();
    setup.fixture.detectChanges();
    expect(setup.fixture.nativeElement.innerHTML).not.toContain('Hyperlanes component exists');
    expect(setup.fixture.nativeElement.innerHTML).not.toContain('Stars component exists');
  ...
  it('#should throw error if using forChild() without using forRoot()', (async () => {
    ...
    await expectAsync(router.navigate(['planets/m', {outlets: {nestedOutlet: 'species'}} ]))
  ...
  it('#should only have DynamicHooksInheritance.All (and no other inheritance setting) work in sync config', fakeAsync(() => {
    ...
    router.navigate(['planets/m', {outlets: {nestedOutlet: 'species'}} ]); // PlanetSpeciesComponent normally has DynamicHooksInheritance.None, but is overwritten here
    tick(1000);
    setup.fixture.detectChanges();
