...
Automtically insert live Angular components into a dynamic string of content (based on their selector or **any pattern of your choice**) and render the result in the DOM.
...
6. [Configuration](#6-configuration)
    ...
    * [Outlet component bindings](#outlet-component-bindings)
    * [HookParserEntry](#hookparserentry)
    * [Outlet component options](#outlet-component-options)
    * [Lazy-loading components](#lazy-loading-components)
7. [Writing your own HookParser](#7-writing-your-own-hookparser)
    * [What makes up a parser](#what-makes-up-a-parser)
    * [Example: Emoji parser (standalone)](#example-emoji-parser-standalone)
    * [Example: Internal link parser (enclosing)](#example-internal-link-parser-enclosing)
8. [Trivia](#8-trivia)
...
```sh
npm install ngx-dynamic-component-hooks --save
```

or yarn

```sh
yarn add ngx-dynamic-component-hooks
```
...
#### What is a hook? 
Simply put, hooks are any piece of text in the content string to be replaced by an Angular component. Hooks can be **standalone** (`<hook>`) or **enclosing** (`<hook>...</hook>`). To find them, each hook has a corresponding **HookParser** that looks for it and tells the library how to instantiate the component.

In most use cases, you might simply want to use the existing component selectors as their hooks. This is why this library comes with an out-of-the-box `SelectorHookParser` that is easy to set up. With it, you can write your selectors just like you would in a normal template (`<app-somecomponent [someInput]="'hello!'">...</app-somecomponent>`) and the corresponding components will be loaded in their place.
...
>**Note:** This library does not parse the content string as an actual Angular template. It merely looks for all registered hooks inside of it, automatically replaces them with their corresponding Angular components **using native Angular methods** and then renders the result. This means that special Angular template syntax will not work. On the flipside, this grants a great deal more flexbility and security than just parsing a template, such as allowing components to be loaded by any text pattern, support for both JiT- and AoT-modes, granular control over which components are allowed (and with which inputs), limiting the scope that can be accessed by the author, content sanitization and more.
...
```html
<ngx-dynamic-hooks [content]="'Load component here: <app-example></app-example>'"></ngx-dynamic-hooks>
```
...
See it in action in this [Stackblitz](https://stackblitz.com/edit/ngx-dynamic-hooks-quickstart), including additional hooks with inputs and ng-content. 
...
The context object acts as a bridge between the parent component holding the `OutletComponent` and all dynamically loaded components within. Given a context object like:
...
**Note:** The context object is the full scope of code that is visible from the content string. No variables or functions, global or otherwise, can be accessed outside of it. This is an intentional security measure. Simply put whatever you want to make available to the author of the text into the context object.
...
| context variables (see [previous point](#context-dependency-injection)) | `[inputName]="context.someProp"` |
...
1. Parent components cannot use `@ContentChildren()` to get a list of all of the nested components in the content string, as these have to be known at compile time. However, you can still access them via `onDynamicMount()` (see [Lifecycle methods](#lifecycle-methods)). 
...
* `onDynamicMount()` is called once as soon as **all** dynamic components have rendered (including [lazy-loaded ones](#lazy-loading-components)). It is given an `OnDynamicData`-object as its parameter, containing the context object as well as the content children of the component.
...
The input and output bindings you assign to hooks are checked and updated on every change detection run, which mirrors Angular's default behaviour. This way, if you bind a context property to an input and that property changes, the corresponding component will automatically be updated with the new value for the input, triggering ` ngOnChanges()`. Alternatively, you can also set the option `updateOnPushOnly` to `true` to only update the bindings when the context object changes by reference (see [Outlet component options](#outlet-component-options)).
...
--- | --- | ---
`globalParsers` | `HookParserEntry[]` | An list of hook parsers to provide to all `OutletComponents` (see [HookParserEntry](#hookparserentry))
`globalOptions` | `OutletOptions` | An options object to provide to all `OutletComponents` (see [Outlet component options](#outlet-component-options))

Note that you don't have to define a global settings object. You can also configure each `OutletComponent` with their [own parsers and options](#outlet-component-bindings) as inputs.

### Outlet component bindings:
...
`parsers` | `HookParserEntry[]` | An optional list of hook parsers to use instead of the global parsers (see [HookParserEntry](#hookparserentry))
`options` | `OutletOptions` | An optional options object to use instead of the global options (see [Outlet component options](#outlet-component-options))

There is also an output you may subscribe to:

Output name | Type | Description
--- | --- | ---
`componentsLoaded` | `Observable<boolean>` | Will trigger once all components have loaded (including [lazy-loaded ones](#lazy-loading-components))

### HookParserEntry:
Hooks can only be found if they have a corresponding `HookParser`. You can register `HookParser`s in the [global settings](#global-settings) or [on each OutletComponent](#outlet-component-options) individually. Both expect a `HookParserEntry`-array, which is just a fancy alias for several possible values. A `HookParserEntry` can be either:
...
These mostly determine the details about how the component selector is parsed from the content string.
Property | Required | Type | Default | Description
--- | --- | --- | --- | ---
`component` | yes | `ComponentConfig` | - | The component to be used. Can be its class or a [LazyLoadComponentConfig](#lazy-loading-components).
`name` | no | `string` | - | The name of the parser. Required if you want to black- or whitelist it.
`selector` | no | `string` | The component selector | The selector to use for the hook
`injector` | no | `Injector` | The root injector | The injector to create the component with
`enclosing` | no | `boolean` | `true` | Whether the selector is enclosing (`<app-hook>...</app-hook>`) or not (`<app-hook>`)
`bracketStyle` | no | `{opening: string, closing: string}` | `{opening: '<', closing: '>'}` | The brackets to use for the selector
`parseInputs` | no | `boolean` | `true` | Whether to parse inputs into live variables or leave them as strings
`unescapeStrings` | no | `boolean` | `true` | Whether to remove escaping backslashes from inputs strings
`inputsBlacklist` | no | `string[]` | `null` | A list of inputs to ignore when parsing the selector
`inputsWhitelist` | no | `string[]` | `null` | A list of inputs to exclusively allow when parsing the selector
`outputsBlacklist` | no | `string[]` | `null` | A list of outputs to ignore when parsing the selector
`outputsWhitelist` | no | `string[]` | `null` | A list of outputs to exclusively allow when parsing the selector
`allowContextInBindings` | no | `boolean` | `true` | Whether to allow the use of context object variables in inputs and outputs
`allowContextFunctionCalls` | no | `boolean` | `true` | Whether to allow calling context object functions in inputs and outputs

### Outlet component options:
You can also provide your own `OutletOptions` for each `OutletComponent` and overwrite the default values. These are options determine the overall behavior of the outlet, such as of how the content string is rendered and how dynamic components are managed.
...
`updateOnPushOnly` | `boolean` | `false` | Normally, the bindings of all dynamic components are checked/updated on each change detection run. This setting will have them update only when the context object passed to the `OutletComponent` changes by reference.
...
You may have noticed that the component-property in `SelectorHookParserConfig` has the type `ComponentConfig` (see [HookParserEntry](#hookparserentry)). This means it can be the component class, but also a `LazyLoadComponentConfig`:
...
* `findHooks()` is called once per parser. Its job is to find all of its hooks in the content string and return their positions.
* `loadComponent()` is called once for each hook. Its job is to provide information on how to dynamically create the component for this hook.
...
You really only need to fill out the `component` field, which can be the component class or a `LazyLoadComponentConfig` (see [Lazy-loading components](#lazy-loading-components)). You may optionally also provide your own injector and custom nodes to replace the existing `<ng-content>` of the component (each entry in the outer array represends a `<ng-content>`-slot and the inner array its content).
...
Whatever bindings are returned in this object will be handed to the component. To disallow or ignore inputs/outputs, simply don't include them.
...
### Example: Emoji parser (standalone)
...
### Example: Internal link parser (enclosing)
...
export class DynamicRouterLinkParser implements HookParser {
    ...
    public findHooks(content: string, context: any): Array<HookPosition> {
        // With the regexes we prepared, we can simply use findEnclosingHooks() to retrieve the HookPositions of all
        // internal <a>-elements from the content string
    ...
    public getBindings(hookId: number, hookValue: HookValue, context: any): HookBindings {
        // Our DynamicRouterLinkComponent accepts three inputs: the relative link, an optional queryParams object
        // and an optional anchor fragment, so we have to extract those from our hook / the found <a>-element

        // We can just reuse the hrefAttrRegex here as its first capture group is always the relative part of the url,
        // e.g. '/jedi/windu' from 'https://www.mysite.com/jedi/windu', which is what we're after
        const hrefAttrMatch = hookValue.openingTag.match(this.hrefAttrRegex);
        let relativeLink = hrefAttrMatch[1];

        // The relative part of the link may still contain the query string and the anchor fragment, so we need to split it up accordingly
...
### How it all works:
This library doesn't rely on any special "hacks" to load the dynamic components. Most notably, it uses [ComponentFactory.create()](https://angular.io/api/core/ComponentFactory#create) from Angular's public api, which is safe and has been around since Angular 2.  It then adds a lot of custom code around this core function to render the components at exactly the right place, register inputs and outputs, project the content properly, activate change detection, update and destroy them automatically, etc. - all to integrate the dynamic components into Angular as naturally as possible. If you are curious about the inner workings of the library, here's a short description:
...
2. The `findHooks()`-method of all registered parsers is called and (with the help of the returned `HookPosition[]`s) all found hooks are replaced with component element placeholders.
...
Most notably, it uses Angular's `DOMSanitizer` by default to remove all unsafe HTML in the content string that is not part of a hook. Though not recommended, you may turn this setting off in the [Outlet component options](#outlet-component-options). You will then have to ensure yourself that the rendered content does not include [Cross Site Scripting attacks (XSS)](https://en.wikipedia.org/wiki/Cross-site_scripting) or other malicious code, however.

As mentioned, the `DOMSanitizer` does not actually parse the hooks themselves (as it may remove them depending on their pattern). This is not an issue as the hooks are replaced by components anyway and never actually rendered. Only the corresponding `HookParser` sees the hook in its original form in order to analyze it. It is therefore the parser's responsibility to ensure that whatever malicious code there may be in the hook is not transferred to the rendered component.  For this reason, the standard `SelectorHookParser` that comes with this library does not rely on JavaScript's dangerous `eval()` function to evaluate inputs and outputs and instead utilitzes the included `DataTypeParser` service which internally uses `JSON.parse()` to safely turn strings into variables. **Note:** When writing custom parsers for hooks that take their inputs/outputs directly from the text, make sure to take similar security precautions!

In addition to this, the scope of code that is accessible by the author of the content string is limited to the [context object](#context-dependency-injection), which you can customize to your liking. 

Finally, which components/hooks can be used by the author can be [freely adjusted](#hookparserentry) for each `OutletComponent`, as can their allowed inputs/outputs.
...
3. `@ContentChildren` don't work in dynamically-loaded components, as these have to be known at compile-time. However, you can still access them via [onDynamicMount()](#lifecycle-methods).

### Comparison with similar libraries:
#### [Angular elements](https://angular.io/guide/elements)
Angular elements allows you to register custom HTML elements (like component selectors) with the browser itself that automatically load and host an Angular component when they appear anywhere in the DOM (see [Web components](https://developer.mozilla.org/en-US/docs/Web/Web_Components)) - even outside of the Angular app. For that reason, these elements work in dynamic content as well and may satisfy your needs.

However, there are a number of advantages this library offers compared to Angular elements:

* **Hook pattern flexibility:** You are not limited to load components by their selector HTML tags. A hook can have any form and doesn't have to be an HTML element at all. You can automatically replace anything you want with a component, which opens up many possibilities for user-generated content (see [Emoji-example](#example-emoji-parser-standalone)) or your own posts (see [Internal links example](#example-internal-link-parser-enclosing)).
* **Control:** With Angular elements, you have no say in where the components are allowed be loaded. Web components will automatically load anywhere in- or outside your app as they are globally registered with the browser. With this library however, you can to specify for each `OutletComponent` individually which hooks to look for, what components to load for them and which inputs/outputs give them.
* **Context:** In Angular elements, there is no direct line of communication with the component hosting the dynamic content, such as the [context object](#context-dependency-injection) from this library. You will have to fallback on services to transfer data.
...
You most likely have a typo in the input. If its a string, remember to put quotation marks around it ('', "" or ``). If that isn't it, it may help to copy the input into an IDE that is set to JS/TS syntax have it highlight potential typos for you.

* **In my output function, `this` does not point to the parent object of the function**
See the [Outputs-section](#outputs) for a solution to this problem.

* **The globalParsersBlacklist/whitelist inputs for the `OutletComponent` don't work**
Make sure you have explicitly given the parsers a name (see [Parser entries](#parser-entries) on how to do so) that correlates with the black/whitelisted name.
...
## 10. Special thanks
Thanks to [ng-dynamic](https://github.com/lacolaco/ng-dynamic) for giving me the idea for this library (as well [this blog post](https://www.arka.com/blog/dynamically-generate-angular-components-from-external-html), which explains the idea more).
