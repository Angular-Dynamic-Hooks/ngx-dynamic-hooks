  ...
  processConfig(userParserConfig: GenericSelectorParserConfig): GenericSelectorParserConfig {
    ...
    // If is LazyLoadingComponentConfig
      ...
      if (!userParserConfig.hasOwnProperty('selector')) {
        throw Error(`DynCompHooks: When using lazy-loaded dynamic components, you have to specify the "selector" property in the parser config (that will be used to find it in the text), as the real selector can't be known before the component is loaded.`); 
      }
    // If is neither
    } else {
