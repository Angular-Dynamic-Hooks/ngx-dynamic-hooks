  ...
  private validateHookPositions(parserResults: Array<ParserResult>): Array<ParserResult> {
    ...
    outerloop: for (const [index, parserResult] of parserResults.entries()) {
      ...
      if (hookPos.openingTagStartIndex >= hookPos.openingTagEndIndex) {
        if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions - openingTagEndIndex has to be greater than openingTagStartIndex. Ignoring.', hookPos); }
        continue;
      }
      if (isMultiTag && hookPos.closingTagStartIndex >= hookPos.closingTagEndIndex) {
        if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions - closingTagEndIndex has to be greater than closingTagStartIndex. Ignoring.', hookPos); }
        continue;
      }
      if (isMultiTag && hookPos.openingTagEndIndex > hookPos.closingTagStartIndex) {
        if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions - The closing tag must start after the opening tag has concluded. Ignoring.', hookPos); }
        continue;
      }
      ...
      innerloop: for (const previousHook of previousHooks) {
        ...
        if (
          ...
          if (isDevMode()) { console.warn('DynCompHooks: A hook with the same position as another hook was found. There may be multiple identical parsers active that are looking for the same hook. Ignoring duplicates. \nPrevious: ', hookPos, '\nCurrent:', prevHookPos); }
          continue outerloop;
        }
        ...
        if (hookPos.openingTagStartIndex < prevHookPos.openingTagEndIndex) {
          if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions: Hook opening tag starts before previous hook opening tag ends. Ignoring. \nPrevious: ', hookPos, '\nCurrent:', prevHookPos); }
          continue outerloop;
        }
        ...
        if (prevIsMultiTag && !(
          ...
          if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions: Opening tag of hook overlaps with closing tag of previous hook. Ignoring. \nPrevious: ', hookPos, '\nCurrent:', prevHookPos); }
          continue outerloop;
        }
        ...
        if (prevIsMultiTag && isMultiTag && !(
          ...
          if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions: Closing tag of hook overlaps with closing tag of previous hook. Ignoring. \nPrevious: ', hookPos, '\nCurrent:', prevHookPos); }
          continue outerloop;
        }
        ...
        if (isMultiTag && prevIsMultiTag &&
          ...
          hookPos.closingTagStartIndex >= prevHookPos.closingTagEndIndex
            ...
            if (isDevMode()) { console.warn('DynCompHooks: Error when checking hook positions: The closing tag of a nested hook lies beyond the closing tag of the outer hook. Ignoring. \nOuter: ', hookPos, '\nInner:', prevHookPos); }
            continue outerloop;
        }
