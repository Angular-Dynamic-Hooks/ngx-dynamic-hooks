  ...
  findEnclosingHooks(dynamicText: string, openingTagRegex: RegExp, closingTagRegex: RegExp, includeNested: boolean = true): Array<HookPosition> {
    ...
    allTagsLoop: for (const [index, tag] of allTags.entries()) {
      ...
      if (index > 0 && tag.startIndex < allTags[index - 1].endIndex) {
        if (isDevMode()) {
          console.warn('DynCompHooks: Syntax error - New tag "' + tag.value + '" started at position ' + tag.starIndex + ' before previous tag "' + allTags[index - 1].value + '" ended at position ' + allTags[index - 1].endIndex + '. Ignoring.');
        }
        continue;
      }
      ...
      if (tag.isOpening) {
        ...
        if (openedTags.length === 0) {
          if (isDevMode()) {
            console.warn('DynCompHooks: Syntax error - Closing tag without preceding opening tag found.');
          }
          continue;
        }
    ...
    if (openedTags.length > 0) {
      if (isDevMode()) {
        console.warn('DynCompHooks: Syntax error - Opening tags without corresponding closing tags found.');
      }
    }
