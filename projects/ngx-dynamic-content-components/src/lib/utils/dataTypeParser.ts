...
export class DataTypeParser {
  ...
  evaluateDataTypeFromString(dataTypeString: string, context: {[key: string]: any} = {}, event?: any, unescapeStrings: boolean = true, trackContextVariables: any = {}, allowContextFunctionCalls: boolean = true): any {
    ...
    throw Error('DynCompHooks: Data type for following input was not recognized and could not be parsed: "' + dataTypeString + '"');
  }
  ...
  parseAsJSON(JSONString: string, unescapeStrings: boolean = true): any {
    ...
    if (JSONString.match(/"?__proto__"?\s*:/g)) {
      throw Error('DynCompHooks: Setting the "__proto__" property in a hook input object is not allowed.');
    }
    if (JSONString.match(/"?prototype"?\s*:/g)) {
      throw Error('DynCompHooks: Setting the "prototype" property in a hook input object is not allowed.');
    }
    if (JSONString.match(/"?constructor"?\s*:/g)) {
      throw Error('DynCompHooks: Setting the "constructor" property in a hook input object is not allowed.');
    }
  ...
  loadContextVariable(contextVar: string, context: {[key: string]: any} = {}, event?: any, unescapeStrings: boolean = true, trackContextVariables: any = {}, allowContextFunctionCalls: boolean = true): any {
    ...
    try {
      ...
      throw Error('DynCompHooks: The required context variable "' + this.decodeDataTypeString(contextVar) + '" could not be found in the context object. Returning undefined instead.');
    }
  }
  ...
  fetchContextVariable(contextLevel: any, path: Array<any>): any {
    ...
    if (path[0].value ===  '__proto__') {
      throw Error('DynCompHooks: Accessing the __proto__ property through a context variable is not allowed.');
    }
    if (path[0].value ===  'prototype') {
      throw Error('DynCompHooks: Accessing the prototype property through a context variable is not allowed.');
    }
    if (path[0].value ===  'constructor') {
      throw Error('DynCompHooks: Accessing the constructor property through a context variable is not allowed.');
    }

    if (contextLevel === undefined) {
      throw Error('DynCompHooks: Context variable path could not be resolved. Trying to access ' + (path[0].type === 'property' ? 'property "' + path[0].value + '" of undefined.' : 'undefined function.'));
    }
